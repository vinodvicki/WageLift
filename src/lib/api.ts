...
export interface CPICalculationResponse {
  ...
  months_without_raise: number;
  cpi_data: {
    current: number;
    at_last_raise: number;
    difference: number;
  };
...
class WageLiftAPI {
  ...
  // CPI Calculation (Updated to match backend)
  async calculateCPIGap(formData: SalaryFormData): Promise<CPICalculationResponse> {
    return this.request<CPICalculationResponse>('/api/v1/cpi/calculate-gap', {
      method: 'POST',
      body: JSON.stringify({
  ...
  // Salary Benchmarking (Updated to match backend)
  async getBenchmarkData(
    jobTitle: string,
    location: string
  ): Promise<BenchmarkData> {
    return this.request<BenchmarkData>(
      `/api/v1/benchmark/salary-percentiles?job_title=${encodeURIComponent(jobTitle)}&location=${encodeURIComponent(location)}`
    );
  }

  // AI Letter Generation (Updated to match backend)
  async generateRaiseLetter(request: RaiseLetterRequest): Promise<RaiseLetterResponse> {
    return this.request<RaiseLetterResponse>('/api/v1/raise-letter/generate', {
      method: 'POST',
      body: JSON.stringify(request),
    });
  }

  // Email Sending (Updated to match backend)
  async sendRaiseLetter(
    letterContent: string,
    subjectLine: string,
    managerEmail: string
  ): Promise<{ success: boolean; message: string }> {
    return this.request<{ success: boolean; message: string }>('/api/v1/email/send-raise-letter', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async saveSalaryEntry(salaryData: SalaryFormData): Promise<any> {
    return this.request('/api/v1/salary/entry', {
      method: 'POST',
  ...
  async getUserSalaryEntries(): Promise<any[]> {
    return this.request('/api/v1/salary/entries');
  }
}
